import React, { Component } from 'react'
import Testifieritem2 from './Testifieritem2'

class Testifiers2 extends Component {
    state = {
        contents: [
            {
                id: "1",
                avatar_pic: "avatar2-pics/Ellipse 22.png",
                content: "Temi Obadofin",
                title: "vendor",
                textContent: <p>
                                Aliqua id fugiat nostrud irure ex duis ea quis id quis ad et. 
                                Sunt qui esse pariatur duis deserunt mollit dolore cillum minim tempor enim. 
                                Elit aute irure tempor cupidatat incididunt sint deserunt ut voluptate aute id deserunt nisi. 
                                Aliqua id fugiat nostrud irure ex duis ea quis id quis ad et. Sunt qui esse pariatur duis deserunt mollit dolore cillum minim tempor enim.
                            </p>
            },
            {
                id: "2",
                avatar_pic: "avatar2-pics/Ellipse 23.png",
                content: "Promise Ejiofor",
                title: "vendor",
                textContent: <p>
                                Aliqua id fugiat nostrud irure ex duis ea quis id quis ad et. 
                                Sunt qui esse pariatur duis deserunt mollit dolore cillum minim tempor enim. 
                                Elit aute irure tempor cupidatat incididunt sint deserunt ut voluptate aute id deserunt nisi.
                                Amet minim mollit non deserunt ullamco est sit aliqua dolor do amet sint. Velit officia consequat duis enim velit mollit. Exercitation veniam consequat sunt nostrud amet.
                            </p>
            },
            {
                id: "3",
                avatar_pic: "avatar2-pics/Ellipse 24.png",
                content: "Feyisola Arinola",
                title: "vendor",
                textContent: <p>
                                Aliqua id fugiat nostrud irure ex duis ea quis id quis ad et. 
                                Sunt qui esse pariatur duis deserunt mollit dolore cillum minim tempor enim. 
                                Elit aute irure tempor cupidatat incididunt sint deserunt ut voluptate aute id deserunt nisi. 
                                Elit aute irure tempor cupidatat incididunt sint deser unt ut voluptate aute id deserunt nisi. Amet minim mollit non deserunt ullamco est sit aliqua dolor do amet sint.   
                            </p>
            },
            {
                id: "4",
                avatar_pic: "avatar2-pics/Ellipse 22b.png",
                content: "Karen Ibeh",
                title: "vendor",
                textContent: <p>
                                Aliqua id fugiat nostrud irure ex duis ea quis id quis ad et. 
                                Sunt qui esse pariatur duis deserunt mollit dolore cillum minim tempor enim. 
                                Elit aute irure tempor cupidatat incididunt sint deserunt ut voluptate aute id deserunt nisi. 
                                Aliqua id fugiat nostrud irure ex duis ea quis id quis ad et. Sunt qui esse pariatur duis deserunt mollit dolore cillum minim tempor enim.
                            </p>
            },
            {
                id: "5",
                avatar_pic: "avatar2-pics/Ellipse 23b.png",
                content: "Oluchi Uzo",
                title: "vendor",
                textContent: <p>
                                Aliqua id fugiat nostrud irure ex duis ea quis id quis ad et. 
                                Sunt qui esse pariatur duis deserunt mollit dolore cillum minim tempor enim. Elit aute irure tempor cupidatat incididunt sint deser <br /> unt ut voluptate aute id deserunt nisi. 
                                Amet minim mollit non deserunt ullamco est sit aliqua dolor do amet sint. 
                                Velit officia consequat duis enim velit mollit. Exercitation veniam consequat sunt nostrud amet.


                            </p>
            },
            {
                id: "6",
                avatar_pic: "avatar2-pics/Ellipse 24b.png",
                content: "Amos Okafor",
                title: "vendor",
                textContent: <p>
                                Aliqua id fugiat nostrud irure ex duis ea quis id quis ad et. 
                                Sunt qui esse pariatur duis deserunt mollit dolore cillum minim tempor enim. 
                                Elit aute irure tempor cupidatat incididunt sint deserunt ut voluptate aute id deserunt nisi. 
                                Elit aute irure tempor cupidatat incididunt sint deser unt ut voluptate aute id deserunt nisi. Amet minim mollit non deserunt ullamco est sit aliqua dolor do amet sint.
                            </p>
            }

        ]
    }
    render() {
        return (
            <div style={structure}>
                <div style={testifierStyle}>
                 {this.state.contents.map( user => (
                    <Testifieritem2 key={user.id} user={user} />
                 ))}
                </div>
            </div>
        )
    }
}

const structure = {
    margin: "8rem 8.5rem"
}
const testifierStyle = {
    display: "grid",
    gridTemplateColumns: "repeat(3, 1fr)",
    gridGap: "1rem"
}

export default Testifiers2
